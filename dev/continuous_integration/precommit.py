...
def main() -> int:
    ...
    if Step.PYLINT in selects and Step.PYLINT not in skips:
        ...
        pylint_rc = pathlib.Path("dev/continuous_integration") / "pylint.rc"

        exit_code = call_and_report(
            verb="pylint",
            cmd=[sys.executable, "-m", "pylint", f"--rcfile={pylint_rc}"]
            + pylint_targets,
            cwd=repo_root,
        )
        if exit_code != 0:
    ...
    if Step.DOCTEST in selects and Step.DOCTEST not in skips:
        ...
        for pth in (repo_root / "express_schema").glob("**/*.py"):
    ...
    if (
        ...
        print("Checking that express_schema/__init__.py and pyproject.toml coincide...")
        exit_code = call_and_report(
            verb=("check that express_schema/__init__.py and pyproject.toml coincide"),
            cmd=[
                sys.executable,
        ...
        print(
            "Skipped checking that express_schema/__init__.py "
